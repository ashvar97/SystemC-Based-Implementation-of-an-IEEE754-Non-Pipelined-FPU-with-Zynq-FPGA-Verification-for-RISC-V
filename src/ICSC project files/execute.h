SC_MODULE(Execute) {
    ...
    sc_signal<bool> div_done;
    sc_signal<bool> div_in_progress;
    ...
    void execute_process() {
        ...
        div_start.write(false);
        div_in_progress.write(false);
        ...
        while (true) {
            if (reset.read()) {
                ...
                if (div_in_progress.read() && div_done.read()) {
                    div_in_progress.write(false);
                    div_start.write(false);
                }
                
                // Check if we need to start a new division
                bool start_new_div = (valid_in.read() && reg_write_in.read() && 
                                    (opcode.read() == 0x0C) && !div_in_progress.read());
                
                if (start_new_div) {
                    div_start.write(true);
                    div_in_progress.write(true);
                } else {
                    div_start.write(false);
                }
                
                // Output logic - only output when not waiting for division
                bool can_output = !div_in_progress.read() || 
                                (div_in_progress.read() && div_done.read());
    ...
    SC_CTOR(Execute) : sub_enable("sub_enable"), div_start("div_start"), 
                       div_done("div_done"), div_in_progress("div_in_progress") {
        ...
        fp_divider->clk(clk);           // Connect clock
        fp_divider->start(div_start);   // Connect start signal
        fp_divider->result(fp_div_result);
        fp_divider->done(div_done);     // Connect done signal
