...
SC_MODULE(FPPipelinedProcessor) {
    ...
    sc_signal<sc_uint<8>> decode_pc_out;  // ✅ NEW: Decode PC
    ...
    sc_signal<sc_uint<8>> ex_pc_out;  // ✅ NEW: Execute PC
        ...
        while (true) {
            if (reset.read()) {
                ...
                if (terminated && pc_out.read() >= 16) {
                    ...
                    for (int i = 1; i <= 19; i++) {
                        if (i <= 11 || (i >= 16 && i <= 19)) {
                            cout << "f" << i << ": 0x" << hex << reg_file[i].read() << dec << endl;
                        }
                    }
    ...
    void decode_process() {
        ...
        decode_pc_out.write(0);  // ✅ Initialize PC
        ...
        while (true) {
            if (reset.read()) {
                ...
                decode_pc_out.write(pc_out.read().range(7, 0));  // ✅ Capture PC
                ...
                if (ifu_valid_out.read() && ifu_instruction_out.read() != 0) {
                    ...
                    cout << "PC=0x" << hex << pc_out.read().range(7, 0) << dec;
        ...
        while (true) {
            if (reset.read()) {
                ...
                if (rd_index < 32) {
                    ...
                    cout << "PC=0x" << hex << wb_pc_out.read() << dec;
                    cout << " f" << rd_index << " updated to 0x" << hex << wb_result_out.read() << dec << endl;
                }
            }
            wait();
        }
    ...
    void update_monitor() {
        // ✅ FIXED: Now uses WB stage PC instead of fetch PC
        monitor_valid.write(wb_valid_out.read());
        monitor_pc.write(wb_pc_out.read());
    }
        ...
        writeback("writeback")
    {
        ...
        execute.pc_in(decode_pc_out);  // ✅ Connect PC
        ...
        execute.pc_out(ex_pc_out);  // ✅ Connect PC

        memory.clk(clk);  // ✅ Connect clock
        memory.reset(reset);
        ...
        memory.pc_in(ex_pc_out);  // ✅ Connect PC
        ...
        memory.pc_out(mem_pc_out);  // ✅ Connect PC

        writeback.clk(clk);  // ✅ Connect clock
        ...
        writeback.pc_in(mem_pc_out);  // ✅ Connect PC
        ...
        writeback.pc_out(wb_pc_out);  // ✅ Connect PC

        SC_METHOD(update_opcode);
        sensitive << decode_instruction_out;
        
        SC_METHOD(update_monitor);
        sensitive << wb_valid_out << wb_pc_out;  // ✅ Sensitive to WB PC
...
int sc_main(int argc, char* argv[]) {
    ...
    cout << "\n================ Floating-Point Processor Simulation ================\n" << endl;
