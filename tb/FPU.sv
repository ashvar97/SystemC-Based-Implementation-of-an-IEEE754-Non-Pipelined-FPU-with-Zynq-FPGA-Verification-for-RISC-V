// Fixed FPPipelinedProcessor with proper pipeline stages
...
logic [7:0] decode_pc_out;  // NEW: Track PC through pipeline
logic [6:0] opcode;
...
logic [7:0] ex_pc_out;  // NEW: Track PC through pipeline
logic [31:0] mem_result_out;
logic [4:0] mem_rd_out;
logic mem_reg_write_out;
logic mem_valid_out;
logic [31:0] mem_instruction_out;
logic [7:0] mem_pc_out;  // NEW: Track PC through pipeline
logic [31:0] wb_result_out;
logic [4:0] wb_rd_out;
logic wb_reg_write_en;
logic wb_valid_out;
logic [7:0] wb_pc_out;  // NEW: Track PC through pipeline
...
//------------------------------------------------------------------------------
// Method process: update_monitor - FIXED!
always_comb 
begin : update_monitor
    monitor_valid = wb_valid_out;
    monitor_pc = wb_pc_out;  // ✅ Now uses WB stage PC!
end

//------------------------------------------------------------------------------
// Clocked THREAD: ifu_process
logic terminated;
...
always_comb begin : ifu_process_comb
    ifu_process_func;
end
...
function void ifu_process_func;
    ...
    if (reset) begin
        ...
        if (!internal_stall && !terminated_next) begin
            current_pc = pc_next;
...
//------------------------------------------------------------------------------
// Clocked THREAD: decode_process - FIXED to propagate PC
...
logic [7:0] decode_pc_out_next;

always_comb begin : decode_process_comb
    decode_process_func;
end
...
function void decode_process_func;
    ...
    decode_pc_out_next = decode_pc_out;
    ...
    if (reset) begin
        ...
        decode_pc_out_next = 0;
    end else begin
        if (!internal_stall) begin
            decode_valid_out_next = ifu_valid_out;
            decode_instruction_out_next = ifu_instruction_out;
            decode_pc_out_next = pc_out[7:0];  // ✅ Capture PC here
            
            if (ifu_valid_out && ifu_instruction_out != 0) begin
...
begin : decode_process_ff
    if (reset) begin
        ...
        decode_pc_out <= decode_pc_out_next;
    end
end
...
always_comb begin : reg_file_update_comb
...
begin : reg_file_update_ff
    if (reset) begin
        // Initialization handled in testbench
    end else begin
...
Execute execute (
  ...
  .pc_in(decode_pc_out),  // ✅ Pass PC through
...
(
    ...
    input logic [7:0] pc_in,  // ✅ NEW
    output logic [31:0] result_out,
    output logic [4:0] rd_out,
    output logic reg_write_out,
    output logic valid_out,
    output logic [31:0] instruction_out,
    output logic [7:0] pc_out  // ✅ NEW
);
...
logic [7:0] pc_out_next;

always_comb begin : execute_process_comb
    ...
    if (reset) begin
        ...
        if (!stall) begin
            ...
            pc_out_next = pc_in;  // ✅ Propagate PC
            
            if (valid_in && reg_write_in) begin
                case (opcode)
                    0: result_out_next = fp_add_result;
                    4: result_out_next = fp_sub_result;
                    8: result_out_next = fp_mul_result;
                    12: result_out_next = fp_div_result;
                    default: result_out_next = 0;
                endcase
            end
        end
    end
...
//==============================================================================
// Memory - FIXED to be sequential!
//==============================================================================
module Memory
(
    input logic clk,  // ✅ NOW CLOCKED
    input logic reset,
    input logic stall,
    input logic valid_in,
...
always_comb begin : memory_process_comb
    result_out_next = result_out;
    rd_out_next = rd_out;
    reg_write_out_next = reg_write_out;
    valid_out_next = valid_out;
...
//==============================================================================
// Writeback - FIXED to be sequential!
//==============================================================================
module Writeback
(
    input logic clk,  // ✅ NOW CLOCKED
...
logic [31:0] result_out_next;
logic [4:0] rd_out_next;
logic reg_write_en_next;
logic valid_out_next;
logic [7:0] pc_out_next;

always_comb begin : writeback_process_comb
    logic do_write;
    
    result_out_next = result_out;
    rd_out_next = rd_out;
    reg_write_en_next = reg_write_en;
    valid_out_next = valid_out;
    pc_out_next = pc_out;
...
begin : writeback_process_ff
    if (reset) begin
        ...
        result_out <= result_out_next;
        rd_out <= rd_out_next;
        reg_write_en <= reg_write_en_next;
        valid_out <= valid_out_next;
        pc_out <= pc_out_next;
    end
end
...
//==============================================================================
//
// Module: ExtractModule (IEEE754Div.h:132:5)
//
...
//------------------------------------------------------------------------------
// Method process: extract (IEEE754Div.h:13:5) 

always_comb 
begin : extract     // IEEE754Div.h:13:5
    if (reset)
    begin
        a_significand = 0;
        b_significand = 0;
...
//==============================================================================
//
// Module: ComputeModule (IEEE754Div.h:133:5)
//
...
//------------------------------------------------------------------------------
// Method process: compute (IEEE754Div.h:48:5) 

always_comb 
begin : compute     // IEEE754Div.h:48:5
    logic [31:0] r;
